...
case " ${EVAL_TOOLS_EFFECTIVE} " in
  *" snyk "*)
    ...
    if command -v timeout >/dev/null 2>&1; then
      SNYK_TIMEOUT_BIN="$(command -v timeout)"
    elif command -v gtimeout >/dev/null 2>&1; then
      SNYK_TIMEOUT_BIN="$(command -v gtimeout)"
    else
      echo "ERROR: neither timeout nor gtimeout is installed"
      exit 1
    fi

    export SNYK_BIN="$(command -v snyk)"
    export TIMEOUT_BIN="$SNYK_TIMEOUT_BIN"
    export SNYK_TIMEOUT_SECONDS="${SNYK_TIMEOUT_SECONDS:-120}"

    SNYK_RUNTIME_WRAPPER="${EXPERIMENT_DIR}/evaluate_snyk_runtime.sh"

    cat > "$SNYK_RUNTIME_WRAPPER" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
    ...
    chmod +x "$SNYK_RUNTIME_WRAPPER"
    export SNYK_BASH_SCRIPT="$SNYK_RUNTIME_WRAPPER"

    log "Snyk CLI available"
    log "Snyk timeout binary: $TIMEOUT_BIN"
    log "Snyk wrapper prepared: $SNYK_BASH_SCRIPT"
    ;;
esac
