...
print(hashlib.sha256(json.dumps(payload, separators=(",", ":"), ensure_ascii=False).encode("utf-8")).hexdigest())
PY
}
...
try:
    from evaluation.analysis.plots import plot_tool_comparison as external_plot_tool_comparison
except Exception:
    external_plot_tool_comparison = None


def fallback_plot_tool_comparison(agg, out_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "tool_comparison.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
...
if external_plot_tool_comparison is not None:
    external_plot_tool_comparison(agg, str(experiment_dir))
else:
    fallback_plot_tool_comparison(agg, experiment_dir)
EOF
