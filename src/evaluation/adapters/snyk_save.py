...
log = logging.getLogger("adapters.snyk")
...
class SnykAdapter(VulnerabilityToolAdapter):
    ...
    def load_findings_for_component(
        ...
        # SBOM scan is global; filter later if needed
        return []
    ...
    def _extract_findings(self, data: Dict[str, Any]) -> List[Finding]:
        ...
        for v in self.iter_with_progress(
            ...
            # --------------------------------------------------
            # Ecosystem inference (must come BEFORE normalization)
            # --------------------------------------------------
            ...
            # --------------------------------------------------
            # Component normalization (Maven-specific fix)
            # --------------------------------------------------
            ...
            # RULE: require at least one identifier
            ...
            # --------------------------------------------------
            # Optional affected version range
            # --------------------------------------------------
