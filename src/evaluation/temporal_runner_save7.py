...
def run_temporal(gt_path: str, sbom_path: str | None, output_dir: str) -> None:
    ...
    run_started_at = datetime.now().astimezone().strftime("%Y-%m-%d %H:%M:%S %Z")
    log.info("STARTED | %s", run_started_at)
    ...
    run_finished_at = datetime.now().astimezone().strftime("%Y-%m-%d %H:%M:%S %Z")
    log.info("Temporal evaluation completed in %.2fs", duration_seconds)
    log.info("Run-level artifacts written to: %s", run_dir)
    log.info("FINISHED | %s | duration=%.2fs", run_finished_at, duration_seconds)
